...
Arbostar: an offline-first quote/estimate builder for a tree service company (single-tenant — one deployment per company, with per-user data isolation: every user owns their clients, quotes, invoices, and timesheet entries; `admin`-role users manage other users, toggle per-user feature flags, and may browse the app "viewing as" another user, which records an `AuditLog` row with the real admin as `actorId` and the viewed user as `targetUserId`). Staff build a quote (client info, priced line items, photos) that must survive filling out a form with no internet and a page reload mid-fill, then send it to the client for approval via a public link. Full design context lives in `docs/superpowers/specs/` and `docs/superpowers/plans/` — the plan file's "Task 15 addendum" and "Post-plan additions" sections document several real bugs found only by production-build/E2E testing; read them before assuming an area is solid.
...
Email sends through Microsoft Graph (`src/lib/graphMail.ts`), not SMTP — an Azure AD app registration authenticates with the client-credentials flow (`AZURE_TENANT_ID`/`AZURE_CLIENT_ID`/`AZURE_CLIENT_SECRET`, app-only, no user sign-in) and calls `POST /users/{AZURE_SENDER_EMAIL}/sendMail`. The app registration needs application permission `Mail.Send`, admin-consented, and `AZURE_SENDER_EMAIL` must be a real mailbox it's allowed to send as. Replaced a plain SMTP relay (first Gmail with an app password, then SendGrid) after Gmail's personal-account rate-limiting/greylisting caused hours-long delivery delays under real usage — Graph has no such delay. Because Graph's `sendMail` only accepts a single body content type, there's no separate plain-text alternative part anymore — every email is HTML-only (see `sendGraphMail`'s `SendGraphMailOptions`, no `text` field).

The client-facing portal's "From" party block (company name/phone/email/address + logo) reads the quote owner's `CompanyProfile` — one profile per user (`src/lib/companyProfile.ts`, `getCompanyProfile(userId)` upserts by the owner's id), so each user's documents carry their own billing identity. Edited from `/profile` (owner-scoped `GET`/`PATCH /api/company`; an admin in view-as mode edits the viewed user's profile and the mutation is audited); the logo is uploaded via `POST /api/company/logo` and — like quote photos — written to a top-level `uploads/company/` directory and served through `/api/uploads/company/[filename]/route.ts`, not `public/`, for the same "next start only scans public/ once at boot" reason. Uploading a new logo deletes the previous file so orphaned images don't accumulate. Test cleanup note: integration tests create their own user + profile rows and delete ONLY those rows — never delete the shared `'company'`-id profile (the seeded admin's own row) or the admin user.

### Staff notifications (profile page)

`/profile` (session-protected, in `middleware.ts`'s matcher) lets a user change their password (`POST /api/profile/password`, verifies `currentPassword` via bcrypt before hashing the new one) and set `User.notificationEmail` (`GET`/`PATCH /api/profile`) — where client-response emails go, separate from their login email. A blank/`null` value means notifications are off, not "fall back to the login email" (an earlier version silently defaulted to the login email, which meant a user could never actually opt out — fixed rather than documented around). Both `/api/portal/[token]/respond` (approve/decline) and `/api/portal/[token]/booking/respond` (confirm/reject a proposed date) send a notification to `quote.createdBy.notificationEmail`, but only `if (quote.createdBy.notificationEmail)` — skipped entirely when unset — after the status change commits, wrapped in try/catch the same way the client-facing emails are — a notification failure never turns a successful client action into a 5xx. The header's user-email dropdown (Profile / Sign out) reads the session via `useSession()` inside a client-side `<SessionProvider>` (`src/components/Providers.tsx`), not `getServerSession()` in the root layout — the layout doesn't re-render on the client-side navigation after login, so a server-fetched session there stays stuck at "logged out" until a hard reload. `SessionProvider` also avoids forcing every route dynamic, which `getServerSession()` in the root layout did (Next marks the whole tree dynamic once a layout reads cookies), undoing the SW's ability to treat `/quotes` and `/quotes/new` as static.
...
- Ownership: `Client.userId`, `Quote.createdById`, `Invoice.userId`, and `TimesheetEntry.userId` are the data owner for every authenticated read/write (resolved through `requireUserScope()` in `src/lib/userScope.ts`, which honors the admin-only `arbostar-view-as-user` HttpOnly cookie). `Client.email` is unique per user (`@@unique([userId, email])`), not globally — client resolution is upsert keyed by `{ userId, email }`, never a racy find-then-create. Feature flags (`UserFeatureFlag` per `(userId, feature)`) gate the `invoices`, `timesheet`, and `clients_crm` surfaces; quotes are always enabled. Pages/APIs enforce flags server-side; the header only hides links.
- Money/rate fields are Prisma `Decimal`; convert with `Number(x)` before arithmetic or JSON responses. Timesheet math (`src/lib/timesheetMath.ts`) uses `Prisma.Decimal` for persisted totals; `TimesheetEntry.hourlyRate` is a snapshot of `User.hourlyRate` at creation and is never rewritten.
...
- Integration tests (`tests/integration/`) mock `next-auth`'s `getServerSession` via `vi.mock('next-auth', ...)` (plus `next/headers` `cookies()` where routes use `requireUserScope`) but hit a real Postgres DB through Prisma — clean up created rows in `afterAll`. Since `CompanyProfile` became per-user, tests create their own user + profile rows and delete ONLY those rows (`companyProfile.deleteMany({ where: { userId } })` before `user.delete`); never delete the seeded admin (`admin@tiptoptreesltd.com`) or its `'company'`-id profile — that row is the same one a real deployment's `/profile` page edits, so a careless cleanup wipes real company data every time the suite runs against a dev DB with real data in it (which happened once while building `tests/integration/company-api.test.ts`). Logo files created by tests must be removed individually (track the names), never `rmSync` the whole `uploads/company/` directory.
